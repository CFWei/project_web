...
function loadpage(DivName,LoadPage,parameter)
{
	...
	{	
		...
		$(DivName).load(LoadPage,function(){
			...
			$("#content").animate({
					...
					if(parameter==1)
						$('#ItemTable').fadeIn(400);
					else{
						$('#Type2Block').fadeIn(400);
					}
	...
	else if(DivName=="#content"&&LoadPage=="register.php"){
		$(DivName).load(LoadPage,function(){
			$('#RegisterPage').hide();
			$("#content").animate({
					height:"60%",
					width:"80%"
				      },800,function(){$('#RegisterPage').fadeIn(400);});
		});
	...
	else if(DivName=="#content"&&LoadPage=="StoreSetting.php")
	{
		...
		$(DivName).load(LoadPage,{},function(){
			$('#StoreSettingTable').hide();		
			$("#content").animate({
					height:"300px",
					width:"800px",
				      },800,function(){$('#StoreSettingTable').fadeIn(400);});
