<?php
	...
	$query="SELECT custom_id,store,item,number,SelectItem FROM  custom_information where `custom_id` = '".$custom_id."' and life='0'";
	$db->query($query);
	$db->get_num_rows();
	...
	{	
		...
		$query="SELECT StoreName,GPS_Longitude,GPS_Latitude,StoreType FROM store_information where `SerialNumbers` = '".$output[$i]['store']."'";
		$db->query($query);
